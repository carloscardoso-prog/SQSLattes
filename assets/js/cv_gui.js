(function($) {
    ...
    String.prototype.removeAccents = function() {
        return (this.replace(new RegExp("[áàâäã]", "ig"), "a").replace(new RegExp("[éèêë]", "ig"), "e").replace(new RegExp("[íìîï]", "ig"), "i").replace(new RegExp("[óòôöõ]", "ig"), "o").replace(new RegExp("[úùûü]", "ig"), "u").replace(new RegExp("[ç]", "ig"), "c"));
    };

    Array.prototype.search = function(value, key) {
        if (jQuery.isFunction(value) != true) {
            var match = [], pattern = new RegExp(value.removeAccents(), "i"), fields, children;
    ...
    Array.prototype.getObj = function(value, key, children) {
        if (jQuery.isFunction(value) != true) {
            if(!value)
                throw new Error("parameter value must be specified");
            if(!key)
                throw new Error("parameter key must be specified");

            var match = null;

            for(var x = 0; x < this.length; x++) {
                if(this[x].__proto__ !== {}.__proto__)
                    return false;
    ...
    $.win = function(p) {
        ...
        var win = {
            ...
            addBtn : function(data) {
                ...
                var btn = $("<span class='botao'><a href='javascript:void(0)'><span></span>" + data.label + "</a></span>");

                                  var pattnerV = /(SALVAR|NOVO|NOVA|CADASTRAR|UTILIZAR|CONFIRMAR|INSERIR|CONCLUIR)/, pattenerX = /(EXCLUIR|APAGAR|CANCELAR|REMOVER)/;
                                    if(pattnerV.exec($(btn).html().toUpperCase()))
                                     $(btn).find("a").parent().addClass("botao").addClass("salvar");
                                    else if(pattenerX.exec($(btn).html().toUpperCase()))
                                     $(btn).find("a").parent().addClass("botao").addClass("excluir");

                if(data.className)
                    btn.find("a").addClass(data.className);
                if(data.callback)
                    btn.bind("click", data.callback);
            ...
            center : function() {
                ...
                var screenW = $(window).width(), winW = win._wrapper.width(), diffW = (screenW - winW), winX = $(window).scrollLeft(), screenY = $(window).height(), winY = win._wrapper.height(), diffY = (screenY - winY), winY = $(window).scrollTop(), isAbsoluteX = /px/.test(p.x), isAbsoluteY = /px/.test(p.y), x = isAbsoluteX ? winX + parseInt(p.x, 10) : (diffW * ((p.x || 0) * 0.01) + winX), y = isAbsoluteY ? winY + parseInt(p.y, 10) : (diffY * ((p.y || 0) * 0.01) + winY);
            ...
            params : function() {
                ...
                if(p.visible)
                    win.show();
                if(p.width)
                    win.setWidth(p.width);
                if(p.title)
                    win.setTitle(p.title);
                if(p.x || p.y)
                    win.center();
                if(p.content)
                    win.setContent(p.content);
                if(p.url)
                    win.request(p.url);
                if(p.buttons)
                    for(var x = 0; x < p.buttons.length; x++)
                    win.addBtn(p.buttons[x]);
            },
            getButtons : function() {
            ...
            move : function(e) {
                var scrollLeft = $(window).scrollLeft(), scrollTop = $(window).scrollTop(), boundX = $(window).width() + scrollLeft - win._wrapper.width(), boundY = $(window).height() + scrollTop - win._wrapper.height(), x = Math.min(boundX, Math.max(scrollLeft, e.pageX - $(win).data("x"))), y = Math.min(boundY, Math.max(scrollTop, e.pageY - $(win).data("y")));

                win._wrapper.css("left", x).css("top", y);
                win.removeListeners();
            },
            fixContentHeight : function() {
                var viewportHeight = Math.max($("html").height(), $(window).height()), height = (viewportHeight - 80);
    ...
    $.tab = function(o, p) {
        ...
        var tab = {
            ...
            next : function() {
                var index = (tab.getSelectedIndex() + 1), max = (tab.getHolders().size() - 1);
            ...
            createHandler : function(o, i) {
                var tabTitle = $(o).attr("title") || "tab " + (i + 1), tabClass = (i == p.selectedIndex) ? "active-tab" : "inactive-tab";
                return $("<span class='" + tabClass + "'><a href='javascript:void(0)'><span>" + tabTitle + "</span></a></span>").bind("click", function() {
    ...
    $.panel = function(p) {
        ...
        var panel = {
            ...
            params : function() {
                if(p.render)
                    panel.render(p.render);
                if(p.title)
                    panel.setTitle(p.title);
                if(p.info)
                    panel.setInfo(p.info);
                if(p.buttons)
                    for(var x = 0; x < p.buttons.length; x++)
                    panel.addBtn(p.buttons[x]);
            },
            init : function() {
                panel.create();
    ...
    $.upload = function(p) {
        ...
        var upload = {
            ...
            validate : function() {
                var isValid = upload.isValid(), isFilled = upload.isFilled();
            ...
            createInput : function() {
                return $("<input type='file'></input").attr("name", p.name).attr("size", p.size).bind({
                    change : upload.validate,
                    mouseleave : upload.validate
                });
            },
            create : function() {
    ...
    $.grid = function(p) {
        ...
        var grid = {
            ...
            moveUp : function(factor) {
                var selected = grid.getSelectedItems(), provider = grid.getProvider(), limit = factor ? 0 + factor : 0, arr = [];

                $.each(selected, function(i, row) {
                    var index = row.rowIndex, data = provider[index];
            ...
            moveDown : function(factor) {
                var selected = grid.getSelectedItems(), provider = grid.getProvider(), limit = factor ? provider.length + factor : provider.length, arr = [];
            ...
            multiSelection : function(e) {
                ...
                if(e.ctrlKey) {
                    ...
                    var newIndex = grid.getRowIndex(row), oldIndex = $(grid).data("lastRowIndex"), min = Math.min(newIndex, oldIndex), max = Math.max(newIndex, oldIndex), rows = grid.getRows();
            ...
            fixHeaderWidth : function() {
                var headerRows = grid._headerHolder.find("tr:eq(0)").find("th"), contentRows = grid._contentHolder.find("tr:eq(0)").find("td");

                $.each(contentRows, function(i, row) {
                    var header = headerRows.eq(i), rowWidth = $(row).width(), borderWidth = header.outerWidth() - header.innerWidth(), width = rowWidth - borderWidth;
            ...
            createBody : function() {
                ...
                $.each(p.provider, function(i, data) {
                    ...
                    $.each(p.columns, function(j, column) {
                        var cell = $("<td></td>"), editFn = (column.editFunction || null), editable = (data["editable"] != null) ? data["editable"] : (editFn != null), renderFn;
                        renderFn = (column.renderFunction ||
                        function() {
                        ...
                        if(column.textAlign)
                            cell.css("text-align", column.textAlign);
            ...
            createHeader : function() {
                var row = $("<tr></tr>"), lastIndex = (p.columns.length - 1);
                ...
                $.each(p.columns, function(i, item) {
                    ...
                    if(item.width)
                        header.css("width", item.width);
                    if(item.headerAlign)
                        header.css("text-align", item.headerAlign);
                    row.append(header);
                });
            ...
            params : function() {
                if(p.render)
                    grid.render(p.render);
                if(p.provider)
                    grid.setProvider(p.provider);
                if(p.url)
                    grid.setUrl(p.url);
            },
            init : function() {
    ...
    $.suggest = function(p) {
        p = $.extend({
            ...
            searchFunction : function(o, suggest) {
                var provider = o.provider, label = suggest.getAttr("label"), children = suggest.getAttr("children"), pattern = suggest.getRegExp(suggest.getQueryString());
                ...
                $.each(provider, function(i, data) {
                    ...
                    if(pattern.test(lb)) {
                        if(pattern.exec(lb)[1])
                            o.equal.push(data);
                        else if(pattern.exec(lb)[2])
                            o.begin.push(data);
                        else if(pattern.exec(lb)[3])
                            o.any.push(data);
                    }

                    if(data[children])
                        suggest.search({
                            provider : data[children],
                            equal : o.equal,
                            begin : o.begin,
                            any : o.any
                        });
            ...
            createItemFunction : function(o, suggest) {
                var data = o.data, qs = suggest.getQueryString(), pattern = new RegExp(qs, "i"), label = suggest.getAttr("label"), lb = data[label].replace(pattern, "<b>" + qs + "</b>");
        ...
        var INPUT_PADDING_TOP = 3, SCROLL_PADDING_RIGHT = 18, IS_INSIDE_LIST = false;
        ...
        var suggest = {
            ...
            fixPosition : function() {
                var left = suggest._input.position().left, top = suggest._input.position().top + suggest._input.outerHeight() + INPUT_PADDING_TOP;

                $(suggest._list).css("left", left).css("top", top);
            },
            fixListScroll : function(i) {
                var itemHeight = suggest.getListItemHeight(), itemOffset = (itemHeight * i), min = suggest._listContent.scrollTop(), max = suggest._listContent.height() + (min - itemHeight), diff = (itemOffset < min) ? -itemHeight : (itemOffset > max) ? itemHeight : 0;

                suggest._listContent.scrollTop(min + diff);
            },
            fixListHeight : function() {
                var excessItems = (suggest.getListItems().size() > p.maxResults), height = excessItems ? p.maxResults * suggest.getListItemHeight() : "auto", width = excessItems ? suggest._listContent.width() + SCROLL_PADDING_RIGHT : "auto";
            ...
            searchURL : function() {
                ...
                var qs = suggest.getQueryString(), data = {};
            ...
            createItem : function(i, data) {
                var item = p.createItemFunction({
                    data : data
                }, suggest), mouseMoveFn = function() {
            ...
            create : function() {
                ...
                suggest._input = $("<input type='text' />").bind({
                    keyup : suggest.userInteraction,
                    blur : suggest.onInputBlur
                ...
                suggest._listContent = $("<div class='suggest-content' />").bind({
                    mouseenter : suggest.onListEnter,
                    mouseleave : suggest.onListLeave
                });
    ...
    $.progress = function(p) {
        ...
        var progress = {
            ...
            params : function() {
                if(p.render)
                    progress.render(p.render);
                if(p.progress != null)
                    progress.setProgress(p.progress);
            },
            init : function() {
                progress.create();
                progress.params();
    ...
    $.tree = function(p) {
        p = $.extend({
            ...
            renderFunction : function(o, tree) {
                var obj = o.provider, str = o.opened ? "<ul>" : "<ul style='display:none'>", label = tree.getAttr("label"), icon = tree.getAttr("icon"), children = tree.getAttr("children");

                for(var x = 0; x < obj.length; x++) {
                    var hasChildren = tree.hasChildren(obj[x]), icon = obj[x][icon] ? obj[x][icon] : "item", dataIndex = (o.dataIndex != null) ? (o.dataIndex + "," + x) : x;
        ...
        var nodeIndex = 0, searchIndex = -1, matchIndex = -1;
        ...
        var tree = {
            ...
            getData : function(index) {
                var arr = index.split(","), l = arr.length, provider = p.provider, children = tree.getAttr("children");
            ...
            search : function(qs) {
                if(qs.length < 2)
                    throw new Error("querystring qs too small");
                var provider = tree.getProvider(), pattern = tree.getRegExp(qs), results = [];
                searchIndex = -1;
                matchIndex = -1;
            ...
            bind : function(selector, event, fn) {
                tree._wrapper.find(selector).unbind(event + ".treeListener").bind(event + ".treeListener", function(e) {
                    var node = (this.tagName.toLowerCase() == "li") ? $(this) : $(this).parent("li"), data = tree.getData(node.attr("dataIndex"));

                    fn(this, data, tree, e);
                });
            },
            openByIndex : function(i, mark) {
                if(i == null || typeof  parseInt(i, 10) != "number")
                    return false;
            ...
            expandAll : function() {
                tree._wrapper.find("a.plus").removeClass("plus").addClass("minus").end().find("a.closed").removeClass("closed").addClass("opened").end().find("ul").not("ul:eq(0)").show();
            },
            collapseAll : function() {
                tree._wrapper.find("a.minus").removeClass("minus").addClass("plus").end().find("a.opened").removeClass("opened").addClass("closed").end().find("ul").not("ul:eq(0)").hide();
            },
            // PRIVATE
            searchEngine : function(o) {
                var children = tree.getAttr("children"), label = tree.getAttr("label");
            ...
            appendMarkup : function() {
                tree._wrapper.empty().append(tree.getMarkup({
                    provider : p.provider,
                    opened : true
                }));
            },
            ...
            expandNode : function(node) {
                var $node = $(node), isParentHidden = $node.parent("ul").is(":hidden");
                ...
                $node.find("> a.icon:eq(0)").removeClass("plus").addClass("minus").end().find("> a.icon:eq(1)").removeClass("closed").addClass("opened").end().find("> ul").show();
            },
            collapseNode : function(node) {
                $(node).find("> a.icon:eq(0)").removeClass("minus").addClass("plus").end().find("> a.icon:eq(1)").removeClass("opened").addClass("closed").end().find("> ul").hide();
            },
            ...
            create : function() {
                tree._wrapper = $("<div class='tree-wrapper'></div>").css({
